...
type Props = { selectedTraits?: Token[] };

const NFTView: React.FC<Props> = ({ selectedTraits = [] }) => {
	return (
						...
						{selectedTraits.map((trait, i) => {
							return (
								<div className="absolute top-0 right-0 left-0 bottom-0" key={i}>
									<Image src={trait.image} alt={`Invetory ${trait.name} Image`} layout="fill" />
								</div>
							);
						})}
					</div>
